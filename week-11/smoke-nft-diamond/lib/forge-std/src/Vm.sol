...
interface VmSafe {
    ...
    /// An RPC URL and its alias_. Returned by `rpcUrlStructs`.
    struct Rpc {
    ...
    /// Returns the RPC url for the given alias_.
    function rpcUrl(
    ...
    function getLabel(
        address account
    ) external returns (string memory currentLabel);
...
interface Vm is VmSafe {
    ...
    /// Expect a call to an address with the specified `msg.value` and calldata, and a *minimum* amount of gas_.
    ...
    /// Expect given number of calls to an address with the specified `msg.value` and calldata, and a *minimum* amount of gas_.
    function expectCallMinGas(
        address callee,
        uint256 msgValue,
        uint64 minGas,
